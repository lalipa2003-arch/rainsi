...
export class LuaEngine {
  ...
  public static async create(options: LuaEngineOptions = {}): Promise<LuaEngine> {
    ...
    const isLuau = version.startsWith('0.');
    const wasmFileName = isLuau ? `luau-${version}.wasm` : `lua-${version}.wasm`;
    ...
    if (options.wasmSource instanceof Uint8Array) {
      ...
      let url: URL;
      if (options.wasmBase) {
        url = new URL(`${options.wasmBase}/${wasmFileName}`, import.meta.url);
      } else {
        try {
          url = new URL(`../wasm/${wasmFileName}`, import.meta.url);
        } catch {
          url = new URL(`/${wasmFileName}`, 'http://localhost');
        }
      }
      ...
      if (url.protocol === 'file:') {
        try {
          ...
          throw new Error(`Failed to load ${isLuau ? 'Luau' : 'Lua'} ${version} binary from local file path ${url.pathname}: ${err.message}`);
        }
      } else {
        const resp = await fetch(url.href);
        if (!resp.ok) {
          throw new Error(
            `Failed to load ${isLuau ? 'Luau' : 'Lua'} ${version} binary from default path ${url.href} (HTTP ${resp.status}). ` +
            `Please ensure that the precompiled '${wasmFileName}' file is placed in your project's public/ directory or served at this root URL. ` +
    ...
    if (typeof (instance.exports._initialize as any) === 'function') {
  ...
  private rawSetI(tableIdx: number, n: number): void {
    const exp = this.exports;
    if (this.version.startsWith('5.1') || this.version.startsWith('5.2') || this.version.startsWith('0.')) {
  ...
  public pullLuaValue(idx: number, depth: number = 0): any {
    ...
    const typeNamePtr = exp.lua_typename ? exp.lua_typename(this.L, type) : 0;
    const typeName = typeNamePtr ? this.readString(typeNamePtr) : '';
    ...
    switch (typeName) {
      ...
      case 'table': {
        ...
        while (exp.lua_next(this.L, absTableIdx) !== 0) {
          ...
          const keyTypeNamePtr = exp.lua_typename ? exp.lua_typename(this.L, keyType) : 0;
          const keyTypeName = keyTypeNamePtr ? this.readString(keyTypeNamePtr) : '';
          const val = this.pullLuaValue(-1, depth + 1);

          if (keyTypeName === 'number') {
      ...
      case 'function': {
        ...
        const callable = (...fnArgs: any[]): any => {
          ...
          const status = exp.lua_pcallk ? exp.lua_pcallk(this.L, fnArgs.length, 1, 0, 0, 0) : exp.lua_pcall(this.L, fnArgs.length, 1, 0);
      ...
      case 'vector': {
        if (exp.lua_tovector) {
          const vPtr = exp.lua_tovector(this.L, idx);
          if (vPtr) {
            const f32 = new Float32Array(this.memory.buffer, vPtr, 3);
  ...
  private flushStdio(): void {
    if (this.exports.wasm_flush_stdio) {
      try {
        this.exports.wasm_flush_stdio();
      } catch {}
    }
    this.wasi.flush();
  }
  ...
  public doString(code: string): any {
    ...
    let callStatus = 0;
    try {
      callStatus = exp.lua_pcallk ? exp.lua_pcallk(this.L, 0, -1, 0, 0, 0) : exp.lua_pcall(this.L, 0, -1, 0);
    } catch (e: any) {
      const errMsg = this.readLuaError();
      exp.lua_settop(this.L, 0);
      this.flushStdio();
